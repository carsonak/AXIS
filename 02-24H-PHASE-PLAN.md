...
| H5–10 | First integrated weather → engine → API → IndexedDB → UI loop | ✅ The code path is integrated and covered by fixtures/automated tests. ✅ A sanitized authenticated live response is covered by parser regression tests. 🟡 A deployed physical-phone live loop is unverified. |
| H10–15 | Complete the core farmer journey and offline-first rendering | ✅ Explanation, stage/age, source/confidence, fallback behavior, validation, and responsive screens are implemented. ✅ Today reads IndexedDB before refresh. 🟡 Browser/device offline acceptance is outstanding. |
| H15–19 | Add action/history and selected polish | ✅ Flow-rate minutes, irrigation events, manual flow-meter deltas, seven-day history/chart, rain adjustment, previous-day comparison, multi-plot selection, and settings are implemented. ↪ Existing advice now refreshes hourly while active and supports manual refresh. ⛔ AI remains optional/disabled and sensor adjustment remains gated. |
| H19–22 | Harden, verify, and create a release candidate | ✅ Go tests/vet, frontend typecheck/build, CI, fixture behavior, live payload parsing, and container checks have recorded verification. ⬜ Device cycles, second-device testing, deployed health, recording, and release tagging remain outstanding. |
| H22–24 | Rehearse, record, deploy, and submit | ⬜ No repository evidence confirms rehearsal, deployment, recording, or submission. |
...
- ⬜ Fly configuration/deployment and deployed HTTPS verification.
- ⬜ Physical-phone install/offline cycles, second-device run, rehearsal, recording, and submission.
