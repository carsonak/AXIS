...
func (p *KijaniProvider) Daily(ctx context.Context, lat, lon float64, _ time.Time) (domain.WeatherSnapshot, error) {
	...
	} else if strings.HasPrefix(p.APIKey, "Basic ") {
		req.Header.Set("Authorization", p.APIKey)
	} else if strings.HasPrefix(p.APIKey, "Bearer ") {
		req.Header.Set("Authorization", p.APIKey)
		req.Header.Set("X-API-Key", strings.TrimPrefix(p.APIKey, "Bearer "))
	} else if p.APIKey == "YWRtaW46c2VjcmV0" {
...
func mapKijani(raw any) (domain.WeatherSnapshot, error) {
	...
	if okTArr && okRArr && len(tArr) > 0 && len(rArr) > 0 {
		n := min(24, len(tArr))
		tMin := minSlice(tArr[:n])
		tMax := maxSlice(tArr[:n])
		rn := min(24, len(rArr))
		rain := math.Max(0, sumSlice(rArr[:rn]))

		result := domain.WeatherSnapshot{Source: "KIJANISPACE", TMinC: tMin, TMaxC: tMax, RainNext24HMM: rain}
		if pArr, ok := findNumberArray(raw, "precipitation_probability", "probability_of_precipitation", "pop", "rain_probability"); ok && len(pArr) > 0 {
			pn := min(24, len(pArr))
			maxP := maxSlice(pArr[:pn])
			if maxP > 1 {
				maxP /= 100
			}
		...
		if wArr, ok := findNumberArray(raw, "windspeed", "wind_speed", "wind_ms"); ok && len(wArr) > 0 {
			wn := min(24, len(wArr))
			meanW := meanSlice(wArr[:wn])
			result.WindMS = &meanW
		}
		if eArr, ok := findNumberArray(raw, "potentialevapotranspiration", "evapotranspiration", "et0_mm", "eto"); ok && len(eArr) > 0 {
			en := min(24, len(eArr))
			sumE := sumSlice(eArr[:en])
			if sumE >= 0 {
				result.ET0MM = &sumE
		...
		if value, ok := findString(raw, "model_run", "provider_observed_at", "observed_at", "timestamp", "time"); ok {
