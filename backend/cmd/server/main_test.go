...
func TestDurationEnv(t *testing.T) {
	...
	if got := durationEnv("TEST_DURATION", 20*time.Second, logger); got != 20*time.Second {
		t.Fatalf("invalid duration fallback = %s, want 20s", got)
	}

	t.Setenv("TEST_DURATION", "0s")
	if got := durationEnv("TEST_DURATION", 20*time.Second, logger); got != 20*time.Second {
		t.Fatalf("non-positive duration fallback = %s, want 20s", got)
	}
}
